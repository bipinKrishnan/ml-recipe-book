...
After loading the model, we have to set a value for ```decoder_start_token_id``` and ```pad_token_id```(otherwise the training will throw an error as these are required during training process).
...
Inorder to get good and meaningful captions from the model, we should use beam search instead of greedy search while generating the caption. It's as easy as this:
...
In short, beam search generates more meaningful text compared to greedy search. Also, increasing the number beams for beam search will increase the time taken to generate the text. 
```
...
Now let's create the trainer using the above arguments, model, our dataset and start the training:
...
Now let's see how good is the captions generated by our model. For the time being, we will use some images from the test set we created:
